...
export function mapValue(x: number, in_min: number, in_max: number, out_min: number, out_max: number) {
...
export type TooltipDataAttrs = null | ((value: ContributionChartValue) => Partial<RectProps> | Partial<RectProps>);

class ContributionGraph extends AbstractChart<ContributionGraphProps, ContributionGraphState> {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: ContributionGraphProps) {
    let {maxValue, minValue, valueCache} = this.getValueCache(nextProps.values);
  ...
  getValueCache(values: ContributionChartValue[]) {
    ...
    return {
      valueCache: values.reduce((memo, value) => {
        ...
        const index = Math.floor((date.valueOf() - this.getStartDateWithEmptyDays().valueOf()) / MILLISECONDS_IN_ONE_DAY);
