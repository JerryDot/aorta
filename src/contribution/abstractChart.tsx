...
class AbstractChart<IProps extends AbstractChartProps, IState extends AbstractChartState> extends Component<
...
> {
  calcScaler = (data: number[]) => {
    if (this.props.fromZero) {
      ...
      return Math.max(...data, this.props.fromNumber) - Math.min(...data, this.props.fromNumber) || 1;
    } else {
  ...
  calcHeight = (val: number, data: number[], height: number) => {
    ...
    if (min < 0 && max > 0) {
      ...
      return this.props.fromZero ? height * (val / this.calcScaler(data)) : height * ((val - min) / this.calcScaler(data));
    } else if (min < 0 && max <= 0) {
      return this.props.fromZero ? height * (val / this.calcScaler(data)) : height * ((val - max) / this.calcScaler(data));
    }
  };
  ...
  getPropsForLabels() {
    const {propsForLabels = {}, color, labelColor = color} = this.props.chartConfig;
    return {
      fontSize: 12,
      fill: labelColor(0.8),
      ...propsForLabels,
  ...
  getPropsForVerticalLabels() {
    const {propsForVerticalLabels = {}, color, labelColor = color} = this.props.chartConfig;
    return {
      fill: labelColor(0.8),
  ...
  getPropsForHorizontalLabels() {
    const {propsForHorizontalLabels = {}, color, labelColor = color} = this.props.chartConfig;
    return {
      fill: labelColor(0.8),
  ...
  renderHorizontalLines = config => {
    const {count, width, height, paddingTop, paddingRight, verticalLabelsHeightPercentage = DEFAULT_X_LABELS_HEIGHT_PERCENTAGE} = config;
    ...
    return [...new Array(count + 1)].map((_, i) => {
      ...
      return <Line key={Math.random()} x1={paddingRight} y1={y} x2={width} y2={y} {...this.getPropsForBackgroundLines()} />;
    });
  };

  renderHorizontalLine = config => {
    const {width, height, paddingTop, paddingRight, verticalLabelsHeightPercentage = DEFAULT_X_LABELS_HEIGHT_PERCENTAGE} = config;
    return (
      <Line
        key={Math.random()}
        x1={paddingRight}
  ...
  renderHorizontalLabels = (config: Omit<AbstractChartConfig, 'data'> & {data: number[]}) => {
    const {
      count,
      data,
      height,
    ...
    const {yAxisLabel = '', yAxisSuffix = '', yLabelsOffset = 12} = this.props;
    return new Array(count === 1 ? 1 : count + 1).fill(1).map((_, i) => {
      ...
      const y =
        count === 1 && this.props.fromZero
          ? paddingTop + 4
          : height * verticalLabelsHeightPercentage - (basePosition / count) * i + paddingTop;
      return (
        <Text
  ...
  >) => {
    const {xAxisLabel = '', xLabelsOffset = 0, hidePointsAtIndex = []} = this.props;
    ...
    return labels.map((label, i) => {
      ...
      const x = (((width - paddingRight) / labels.length) * i + paddingRight + horizontalOffset) * fac;

      const y = height * verticalLabelsHeightPercentage + paddingTop + fontSize * 2 + xLabelsOffset;
  ...
  renderVerticalLines = ({
    ...
    Pick<AbstractChartConfig, 'data' | 'width' | 'height' | 'paddingRight' | 'paddingTop' | 'verticalLabelsHeightPercentage'>,
    'data'
  > & {data: number[]}) => {
    const {yAxisInterval = 1} = this.props;

    return [...new Array(Math.ceil(data.length / yAxisInterval))].map((_, i) => {
      return (
        <Line
          key={Math.random()}
  ...
  renderVerticalLine = ({
    ...
    paddingRight,
    verticalLabelsHeightPercentage = DEFAULT_X_LABELS_HEIGHT_PERCENTAGE,
  }: Pick<AbstractChartConfig, 'height' | 'paddingRight' | 'paddingTop' | 'verticalLabelsHeightPercentage'>) => (
    <Line
      key={Math.random()}
  ...
  renderDefs = (
    config: Pick<
      PartialBy<
        AbstractChartConfig,
        'backgroundGradientFromOpacity' | 'backgroundGradientToOpacity' | 'fillShadowGradient' | 'fillShadowGradientOpacity'
      >,
      | 'width'
      | 'height'
    ...
    const {width, height, backgroundGradientFrom, backgroundGradientTo, useShadowColorFromDataset, data} = config;
    ...
    const fillShadowGradient = config.hasOwnProperty('fillShadowGradient') ? config.fillShadowGradient : this.props.chartConfig.color(1.0);

    const fillShadowGradientOpacity = config.hasOwnProperty('fillShadowGradientOpacity') ? config.fillShadowGradientOpacity : 0.1;
    ...
    return (
      <Defs>
        ...
        {useShadowColorFromDataset ? (
          ...
          <LinearGradient id="fillShadowGradient" x1={0} y1={0} x2={0} y2={height} gradientUnits="userSpaceOnUse">
